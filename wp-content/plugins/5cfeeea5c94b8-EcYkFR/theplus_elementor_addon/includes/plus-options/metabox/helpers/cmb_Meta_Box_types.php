...
class cmb_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button" href="#"><?php _e( 'Add Row', 'theplus' ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo $class; ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php _e( 'Remove', 'theplus' ); ?></a>
			</td>
		</tr>
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. __( 'No terms', 'theplus' ) .'</label></li>';
		} else {
			foreach ( $terms as $term ) {
				$args = array(
					'value' => $term->slug,
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => __( 'Add or Upload File', 'theplus' ),
			'name'  => '', 'id'  => '',
		) );
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button">'. __( 'Remove Image', 'theplus' ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						__( 'File:', 'theplus' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'theplus' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. __( 'Remove', 'theplus' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
	...
	public function file() {
		...
		'<input class="cmb_upload_button button" type="button" value="'. __( 'Add or Upload File', 'theplus' ) .'" />',
		$this->_desc( true );
		...
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_media_status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo '<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. __( 'Remove Image', 'theplus' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'theplus' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value, '" target="_blank" rel="external">'. __( 'Download', 'theplus' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. __( 'Remove', 'theplus' ) .'</a>)';
				}
			}
		echo '</div>';
	}
