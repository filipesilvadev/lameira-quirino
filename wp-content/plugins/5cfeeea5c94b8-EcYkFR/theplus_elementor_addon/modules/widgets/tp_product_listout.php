...
class ThePlus_Product_ListOut extends Widget_Base {
    ...
    public function get_title() {
        return __('Woo Product Listing', 'theplus');
    }
	...
	public function get_style_depends() {
		return [ 'tp-columns-bootstrap','plus-product-style' ];
	}
    ...
    protected function _register_controls() {
		...
		$this->add_control(
			'display_product',
			[
				'label' => __( 'Display Product', 'theplus' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'all',
		...
		$this->add_control(
			'display_cart_button',
			[
				'label' => __( 'Cart Button Display', 'theplus' ),
				'type' => Controls_Manager::SWITCHER,
		...
		$this->add_group_control(
			...
			[
				...
				'label' => __( 'Price Typography', 'theplus' ),
		...
		$this->add_control(
			'sale_price_color',
			[
				'label' => __( 'Sale Price Color', 'theplus' ),
				'type' => Controls_Manager::COLOR,
				'default' => '',
				'selectors' => [
		...
		$this->start_controls_section(
            ...
            [
                'label' => __('Product Image', 'theplus'),
                'tab' => Controls_Manager::TAB_STYLE,
            ]
        );
		...
		$this->start_controls_section(
            ...
            [
                'label' => __('Add To Cart Button', 'theplus'),
                'tab' => Controls_Manager::TAB_STYLE,
            ]
        );
		$this->add_responsive_control(
		...
		$this->add_control(
			'quick_view_options',
			[
				'label' => __( 'Quick View Button', 'theplus' ),
				'type' => Controls_Manager::HEADING,
				'separator' => 'before',
				'condition' => [
					'style' => 'style-2',
	 ...
	 protected function render() {
		...
		if ( !class_exists('woocommerce') ) {
			...
			if(!empty($post_category)){
				...
				$post_category=implode(',', $category_product);
			}
			if($settings['post_extra_option']=='pagination' && $layout!='carousel'){
				$output .= theplus_pagination($query->max_num_pages,'2');
					...
					$output .= '<div class="ajax_load_more">';
						$output .= '<a class="post-load-more" data-load="products" data-post_type="product" data-texonomy_category="product_cat" data-load-class="'.esc_attr($uid).'" data-layout="'.$layout.'" data-style="'.esc_attr($style).'" data-desktop-column="'.esc_attr($settings['desktop_column']).'" data-tablet-column="'.esc_attr($settings['tablet_column']).'" data-mobile-column="'.esc_attr($settings['mobile_column']).'" data-metro_column="'.esc_attr($settings['metro_column']).'" data-metro_style="'.esc_attr($metro_style).'" data-responsive_tablet_metro="'.esc_attr($responsive_tablet_metro).'" data-tablet_metro_column="'.esc_attr($tablet_metro_column).'" data-tablet_metro_style="'.esc_attr($tablet_metro_style).'" data-offset-posts="'.($settings['post_offset']).'" data-category="'.esc_attr($post_category).'" data-order_by="'.esc_attr($settings['post_order_by']).'" data-post_order="'.esc_attr($settings['post_order']).'" data-filter_category="'.esc_attr($settings['filter_category']).'" data-display_post="'.esc_attr($settings['display_posts']).'" data-animated_columns="'.esc_attr($animated_columns).'" data-post_load_more="'.esc_attr($settings['load_more_post']).'" data-cart_button="'.esc_attr($display_cart_button).'"  data-page="1" data-total_page="'.esc_attr($query->max_num_pages).'" data-featured_image_type="'.$featured_image_type.'">'.esc_html($settings['load_more_btn_text']).'</a>';					
					$output .= '</div>';					
			}else if($settings['post_extra_option']=='lazy_load' && $layout!='carousel'){
				
					$output .= '<div class="ajax_lazy_load">';
						$output .= '<a class="post-lazy-load" data-load="products" data-post_type="product" data-texonomy_category="product_cat" data-load-class="'.esc_attr($uid).'" data-layout="'.$layout.'" data-style="'.esc_attr($style).'" data-desktop-column="'.esc_attr($settings['desktop_column']).'" data-tablet-column="'.esc_attr($settings['tablet_column']).'" data-mobile-column="'.esc_attr($settings['mobile_column']).'" data-metro_column="'.esc_attr($settings['metro_column']).'" data-metro_style="'.esc_attr($metro_style).'" data-responsive_tablet_metro="'.esc_attr($responsive_tablet_metro).'" data-tablet_metro_column="'.esc_attr($tablet_metro_column).'" data-tablet_metro_style="'.esc_attr($tablet_metro_style).'"  data-offset-posts="'.($settings['post_offset']).'" data-category="'.esc_attr($post_category).'" data-order_by="'.esc_attr($settings['post_order_by']).'" data-post_order="'.esc_attr($settings['post_order']).'" data-filter_category="'.esc_attr($settings['filter_category']).'" data-display_post="'.esc_attr($settings['display_posts']).'" data-animated_columns="'.esc_attr($animated_columns).'" data-post_load_more="'.esc_attr($settings['load_more_post']).'" data-cart_button="'.esc_attr($display_cart_button).'" data-page="1" data-total_page="'.esc_attr($query->max_num_pages).'" data-featured_image_type="'.$featured_image_type.'"><img src="'.THEPLUS_ASSETS_URL. 'images/lazy_load.gif" /></a>';
						$output .= '</div>';
				}
			$output .='</div>';
		}
