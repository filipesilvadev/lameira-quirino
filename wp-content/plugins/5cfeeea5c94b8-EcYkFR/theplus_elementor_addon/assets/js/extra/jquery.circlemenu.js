;(function($, window, document, undefined){
    var pluginName = 'circleMenu',
        defaults = {
            ...
            item_diameter: 30,
            circle_radius: 80,
            circle_radius_tablet: 0,
            circle_radius_mobile: 0,
            angle:{
                start: 0,
                end: 90
            },
    ...
    CircleMenu.prototype.init = function(){
        ...
        self.menu_items.each(function(index){
			...
			}else if(res_width <= 1199 && res_width >= 768){
				if(self.options.circle_radius_tablet != 0 && self.options.circle_radius_tablet!=undefined){
					var x = Math.round(self.options.circle_radius_tablet * Math.cos(angle)),
					y = Math.round(self.options.circle_radius_tablet * Math.sin(angle));	
				}else{
					var x = Math.round(self.options.circle_radius * Math.cos(angle)),
					y = Math.round(self.options.circle_radius * Math.sin(angle));
				}				
			}else if(res_width <= 767){
				if(self.options.circle_radius_mobile != 0 && self.options.circle_radius_mobile!=undefined){
					var x = Math.round(self.options.circle_radius_mobile * Math.cos(angle)),
					y = Math.round(self.options.circle_radius_mobile * Math.sin(angle));
				}else{
					var x = Math.round(self.options.circle_radius * Math.cos(angle)),
					y = Math.round(self.options.circle_radius * Math.sin(angle));
				}
