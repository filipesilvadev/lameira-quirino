var before_after_Container;
var before_after_ContainerId = 0;
var before_after_obj;
var before_after_Separator_obj;
var before_after_sep_Image;
var before_obj;
var after_obj;
var beforeImage;
var afterImage;
var ba_type;
var before_after_sep_show;
var before_after_show_mode;
var changing_this = !1;
var Playing_this = !1;
var sep_Size;
var indSize = 10;
...
function setbefore_after_Container(objFrom) {
    container = jQuery(objFrom).closest(".pt_plus_before_after");
    containerId = container.data("id");
    hide_separator_image(container);
    if (before_after_Container && before_after_ContainerId == containerId) return;
    if (Playing_this) {
        stop_animation();
        if (before_after_sep_show) before_after_Separator_obj.show()
    }
    before_after_Container = container;
    before_after_ContainerId = containerId;
    before_after_Separator_obj = before_after_Container.find(".before-after-sep");
    before_after_obj = before_after_Container.find(".before-after-inner");
    before_obj = before_after_Container.find(".image-before");
    after_obj = before_after_Container.find(".image-after");
    beforeImage = before_after_Container.find(".image-before > img");
    afterImage = before_after_Container.find(".image-after > img");
    ba_type = before_after_Container.data("type");
    before_after_sep_show = !0;
    if (before_after_Container.data("separate_switch") == 'false')
        before_after_sep_show = !1;
    if (before_after_sep_show == !0) before_after_Separator_obj.show();
    else before_after_Separator_obj.hide()
}
...
function onMouseMove(x, y,parent_attr) {
    if (changing_this && before_after_sep_show) before_after_Separator_obj.show();
    ...
    if (ba_type == "horizontal") {
        pos = x - before_after_Container.offset().left;
        if (pos >= before_after_obj.width()) pos = before_after_obj.width();
        before_after_Separator_obj.css("left", pos - (sep_Size / 2));
        if (before_after_show_mode != 0) {
            before_after_sep_Image.css("left", pos)
        }
        ...
        pos = y - before_after_Container.offset().top;
        before_after_Separator_obj.css("top", pos - (sep_Size / 2));
        if (before_after_show_mode != 0) {
            before_after_sep_Image.css("top", pos)
        }
        before_obj.height(pos);
		var before_label=before_obj.find('.before_after_label.before_label_text');
		var bf_obj=before_after_obj.height();
		bf_obj=bf_obj/2;
		if(bf_obj < pos){
			before_obj.find('.before_after_label.before_label_text').css("opacity",'1');		
		}else{
        ...
        pos = x - before_after_Container.offset().left;
        if (pos >= before_after_obj.width()) pos = before_after_obj.width();
        before_after_Separator_obj.css("left", pos - (indSize / 2));
        rat = pos / before_after_obj.width();
...
function hide_separator_image(container) {
    before_after_sep_Image = container.find(".before-after-sep-icon");
    before_after_show_mode = 0;
    if (container.data("separate_image"))
        before_after_show_mode = container.data("separate_image")
}

function show_separator_image() {
    if (before_after_show_mode == 1) {
        before_after_sep_Image.show()
    }
}

function full_After() {
    w = before_after_obj.width() - indSize;
    before_after_Separator_obj.css("left", w);
    beforeImage.css("opacity", "0")
}

function zero_After() {
    before_after_Separator_obj.css("left", 0);
...
function sep_Right() {
    w = before_after_obj.width();
    before_after_Separator_obj.css("left", w - sep_Size / 2);
    if (before_after_show_mode != 0) {
        before_after_sep_Image.css("left", w)
    }
    before_obj.width(w)
}
...
function sep_Left() {
    before_after_Separator_obj.css("left", 0);
    if (before_after_show_mode != 0) {
        before_after_sep_Image.css("left", 0)
    }
    before_obj.width(0)
}

function sep_Top() {
    before_after_Separator_obj.css("top", -sep_Size / 2);
    if (before_after_show_mode != 0) {
        before_after_sep_Image.css("top", 0)
    }
    before_obj.height(0)
}

function sep_Bottom() {
    h = before_after_obj.height();
    before_after_Separator_obj.css("top", h - sep_Size / 2);
    if (before_after_show_mode != 0) {
        before_after_sep_Image.css("top", h)
    }
    before_obj.height(h)
}

function before_after_init() {
    ...
    jQuery(".pt_plus_before_after").each(function() {
        ...
        if (configType != "show") {
            container.on("touchstart", function(event) {
                setbefore_after_Container(this);
                TouchDevice = !0;
                changing_this = !0
            });
        ...
        if (container.data("click_hover_move") == 'on') {
            container.find(".before-after-inner, .before-after-bottom-separate").on("mouseover", function(event) {
                ...
                changing_this = !0;
                if (ba_type == "horizontal") {
                    pos = event.pageX - before_after_Container.offset().left;
                    ...
                    pos = event.pageY - before_after_Container.offset().top - (sep_Size / 2);
            ...
            container.find(".before-after-inner, .before-after-bottom-separate").on("mousedown", function(event) {
                ...
                if (before_after_Separator_obj && before_after_sep_show) before_after_Separator_obj.show();
                setbefore_after_Container(this);
